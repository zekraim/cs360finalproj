...
if ($_SERVER['REQUEST_METHOD'] == "POST") { // if new data has been posted
    ...
    $relation_scheme = sort_str($relation_scheme);
    ...
    if (isset($_POST['dependencies'])) {
        ...
        calcsubsets(str_split($relation_scheme), $subsets, $subset,0);
        usort($subsets, "cmp");
...
if($alldone == true){ // count number of correct superkeys and candidate keys
    ...
    $real_super_count = 0;
    // we already have the lenths of the candidate keys
    foreach(array_slice($closure_sols,1) as $closure){
        if(sort_str($closure['CLOSURE'])==$relation_scheme){ // is a key
...
foreach($to_grade as $answer){
    ...
    // grab the correct closure -- convert to lowercase and sort
    ...
    // grab correct student answer -- convert to lowercase and sort
    $student_answer_sort = sort_str($answer['FD_RIGHT']);
    ...
    if($correct_closure_sort == $student_answer_sort){ // student was correct
...
$json = json_encode(array($relation, $input, $steps, $subsets_done, $correct, $alldone));
    ...
    <body>
        ...
        <script>            
            $(document).ready(function(){                
                disp_sections(display_data[0],display_data[1],display_data[2], display_data[3], display_data[4], display_data[5]);
            });
        </script>
    </body>
</html>