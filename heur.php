...
if ($_SERVER['REQUEST_METHOD'] == "POST") { // if new data has been posted
    if (isset($_POST['relations'])){
        $clear_relation_query = "truncate table relation_scheme";
        mysqli_query($con, $clear_relation_query);
        ...
        // convert to capitals:
        $relation_scheme = strtoupper(implode(explode(',',$_POST['relations'])));
        $query = "insert into relation_scheme (R_SCHEME) values ('$relation_scheme')";
        mysqli_query($con, $query);
       
    }
    $relations_input = "select * from relation_scheme";
    $result_relations = mysqli_query($con, $relations_input);
    $relation_scheme = $result_relations->fetch_all(MYSQLI_ASSOC)[0]['R_SCHEME']; // string with no commas separating
    ...
    if (isset($_POST['dependencies'])) {
        ...
        for ($i = 0; $i < sizeof($dep[0]); $i++) {
            ...
            foreach ($dep[0][$i] as $char) {
                if (in_array($char, str_split($relation_scheme))){
            ...
            if (strlen($fd_left)>0 && strlen($fd_right)>0){
        ...
        $ques_just = array();
        $minus_just = array();
        foreach(str_split($relation_scheme) as $attr){
            $in_left = 0;
            $in_right = 0;
            $attr_plus = array();
            $attr_minus = array();
        ...
        // create the subsets of k_ques and the add contents of k_plus to each subset
        ...
        $k_ques_subsets = array();
        calcsubsets($k_ques, $k_ques_subsets, $subset,0);
        array_push($k_ques_subsets, array(''));
        foreach($k_ques_subsets as $sub){
            array_push($subsets, str_split($k_plus_str . implode($sub)));
        }
        usort($subsets, "cmp");
    ...
    if (isset($_POST['submit_k_forms'])){
        if (
            ...
            $clear_steps_query = "truncate table student_steps";
            mysqli_query($con, $clear_steps_query);
            $clear_student_k_steps_query = "delete from k_sets where ORIGIN = 'STUDENT'";
            ...
            // used explode to get rid of commas, now need to implode to get back into the string
...
// grab relation set and compute candidate key length
...
foreach(array_slice($closure_sols, 1) as $closure){
    if(sort_str($closure['CLOSURE'])==$relation_scheme){ // is a key
...
foreach($to_grade as $answer){
    ...
    $correct_closure_sort = sort_str($correct_closure);
    ...
    if($correct_closure_sort == sort_str($answer['FD_RIGHT'])){ // student was correct
    ...
    // correct label on key type
    $user_key_label = 'INCORRECTLY'; // or incorrectly
    if (strlen($correct_closure_sort) ==  strlen($relation_scheme)){ // if it is a key
        if (strlen($answer['FD_LEFT'])==$candidate_key_length){ // is candidate key
            if($answer['NEXT']=='CANDIDATE'){
                $user_key_label = 'CORRECTLY';
            }
        } else { // is a superkey
            if($answer['NEXT']=='SUPER'){
                $user_key_label = 'CORRECTLY';
            }
        }
    } else { // not a key
    ...
    $feedback[$subset] = array($over_include_steps,$under_include_steps, $invalid_steps, $answer['NEXT'], $user_key_label);
}
...
if(isset($k_sets_student[0])){
    ...
    if(
        sort_str($k_sets_student[0]['ATTR']) == sort_str($k_sets_real[0]['ATTR']) &&
        sort_str($k_sets_student[1]['ATTR']) == sort_str($k_sets_real[1]['ATTR']) &&
        sort_str($k_sets_student[2]['ATTR']) == sort_str($k_sets_real[2]['ATTR'])
        ){
        $k_correct=true;
        $k_form=false;
    } else{
...
$json = json_encode(array($relation, $input, $steps, $subsets_done, $correct, $alldone, $k_form, $k_disp, $k_correct));
...
<html>
    ...
    <body>
        ...
        <div id="relation_form">
            <p>
                Enter the relation scheme you wish to use for your functional dependencies. Each attribute should only be a single character.
                You should separate the attributes by commas. Casing does not matter.
            </p>
            <form action="heur.php" method="post">
                <label for="fname">Input Relation Set:</label><br>
                <input type="text" id="fname" name="relations" class="textinput" placeholder="e.g. a,b,c,d"><br>
                <input type="submit" value="Submit">
            </form> 
        </div>
        <div id="relation_disp">
            <p>You entered the following relation set: {<?php echo implode(',',str_split($relation_scheme))?>}.</p>
        </div>
        <div id="input_form">
            <p>
            ...
            <p>
                ...
                format the system will break. {A,B,...,C,D} are the determinant attributes (only use single characters for an attribute), {W,X,...,Y,Z} are the dependent attributes of each
        ...
        <div id="k_forms"> 
            ...
            Do note use commas for the attributes. Use commas to separate the functional dependency numbers.
            If you are correct, you will be able to begin computing the closures of the K+ and K? sets to find the candidate and super-keys.
        ...
        <div id="k_feedback_incorrect">
            ...
            <p>
                ...
                <b>K+</b>:<?php echo implode(',',array_merge(
                        array_diff(explode(',',$k_sets_student[0]['JUST']),explode(',',$k_sets_real[0]['JUST'])), 
                        array_diff(explode(',',$k_sets_real[0]['JUST']), explode(',',$k_sets_student[0]['JUST']))))?>
                <br>
                <b>K?</b>:<?php echo implode(',',array_merge(
                        array_diff(explode(',',$k_sets_student[1]['JUST']),explode(',',$k_sets_real[1]['JUST'])), 
                        array_diff(explode(',',$k_sets_real[1]['JUST']), explode(',',$k_sets_student[1]['JUST']))))?>
                <br>
                <b>K-</b>:<?php echo implode(',',array_merge(
                        array_diff(explode(',',$k_sets_student[2]['JUST']),explode(',',$k_sets_real[2]['JUST'])), 
                        array_diff(explode(',',$k_sets_real[2]['JUST']), explode(',',$k_sets_student[2]['JUST']))))?>
            </p>
        </div>
        <?php 
        $i=1;
        ...
        ?>
            ...
            <div id="steps_disp_<?php echo $attr;?>">
                ...
                <table border="1">
                    <tr>
                        <th>FD Number</th>
                        <th>Attribute</th>
                        <th>Current Closure</th>
                        <th>FD Justification ID</th>
                        <th>Next</th>
                    </tr>
            ...
            <div id="key_feedback_<?php echo $attr;?>">
                You labeled the attribute set as <?php echo $feedback[$attr][3]?>-KEY <?php echo $feedback[$attr][4]?>.
            </div>
        <?php 
        $i=$i+1;
        endforeach ?>
        ...
        <script>
            $(document).ready(function(){                
                disp_sections(display_data[0],display_data[1],display_data[2], display_data[3], display_data[4],display_data[5],display_data[6],display_data[7],display_data[8]);
            });
        </script>
    </body>
</html>