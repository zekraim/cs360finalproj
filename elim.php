...
if ($_SERVER['REQUEST_METHOD'] == "POST") { // if new data has been posted
    ...
    $relation_scheme = $result_relations->fetch_all(MYSQLI_ASSOC)[0]['R_SCHEME']; // string with no commas separating
        ...
        if (isset($_POST['dependencies'])) {
            ...
            $query_first = "insert into elim_tree (ID,ATTR,LEAF) values ('1','$relation_scheme','0')";
...
foreach(array_slice($closure_sols, 1) as $closure){
    if(strlen($closure['CLOSURE'])==strlen($relation_scheme)){ // is a key
...
foreach(array_slice($closure_sols,1) as $closure){
    if(sort_str($closure['CLOSURE'])==sort_str($relation_scheme)){ // is a key
        if(strlen($closure['ATTR'])==$candidate_key_length){ // candidate key
            array_push($real_candidates, sort_str($closure['ATTR']));
        }
    }
}
...
foreach($user_candidates as $uck){
    array_push($user_candidates_string_arr, sort_str($uck['CANDIDATES']));
}
...
$json = json_encode(array($relation,$input, $tree_disp, $tree_form, $candidates_form, $candidates_disp, $feedback_correct, $feedback_incorrect));
...
<html>
    ...
    <body>
        ...
        <p>
            After you enter in your relation scheme and input functional dependencies, you will create an attribute elimination tree one node at a time.
